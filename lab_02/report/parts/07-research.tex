...
\section*{Вывод}
\begin{enumerate}
	\item Исходя из данных, полученных в таблицах \ref{tbl:time_even}--\ref{tbl:time_even}, можно сделать вывод, что лучше всего работает отмизированный алгоритм Винограда.
	Стандартный же алгоритм умножения матриц работает _ в раз хуже, чем алгоритм Винограда при четном размере строк и в _ раз хуже при нечетном;
	\item Реализация алгоритма Винограда при четном размере матрицы работает в _ раз быстрее, чем при нечетном размере. Это обусловлено наличием дополнительных вычислений
	для крайних строк и столбцов. Поэтому алгоритм Винограда стоит использовать при работе с матрицами, размер которых четный. 
\end{enumerate}
